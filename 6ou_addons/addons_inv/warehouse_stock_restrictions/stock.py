...
class stock_picking(models.Model):
    ...
    location_id = fields.Many2one(
        'stock.location', "Source Location",check_company=True, readonly=True, required=True,
        states={'draft': [('readonly', False)]},
        domain=lambda self: [('id', 'in', self.env.user.stock_location_ids.ids),('usage', '=', 'internal')]
        )
    location_dest_id = fields.Many2one(
        'stock.location', "Destination Location",check_company=True, readonly=True, required=True,
        states={'draft': [('readonly', False)]},
        domain=lambda self: [('id', 'in', self.env.user.stock_location_ids.ids),('usage', '=', 'internal')]
        )